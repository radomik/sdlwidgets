...
/*#include "../srcCommon/file_browse_common.h"*/
#include "Image.h"
#include "Widget.h"
...
bool Image_isExtSupported(char *ext) { /*return fileOnMask(ext, &supported_files);*/ return false; }
...
// Constructor
Image* Image_new(Image *this, char *img_path, usint minx, usint miny) { // png, tiff, giff, jpeg
	if (! this) {
...
void Image_scale(Image *image, double xscale, double yscale, int smooth) { 
...
void Image_setPosition(Image *image, usint minx, usint miny) { Widget_setPosition((Widget*)image, minx, miny); }
