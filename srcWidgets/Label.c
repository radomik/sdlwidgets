...
void Label_setBorder(Label *label, usint bord_width, uint rgb) {
	label->border_width = bord_width;
	label->border       = rgb;
}

void Label_setPadding(Label *label, usint padx, usint pady) {
	label->padx = padx;
	label->pady = pady;
}
...
void Label_setFont(Label *label, TTF_Font *font) 
	{ TextBlock_setFont(&(label->text_block), font); }
	
void Label_setFontColor(Label *label, uint rgb) 
	{ TextBlock_setForegroundColor(&(label->text_block), rgb); /*label->widget.need_refresh = true;*/ }
	
void Label_setBgColor(Label *label, uint rgb) 
