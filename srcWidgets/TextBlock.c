...
void TextBlock_setText(TextBlock *text_block, char *text) {
	usint l = strlen(text);
...
void TextBlock_setFont(TextBlock *text_block, TTF_Font *font)
	{ if ((font) && (text_block)) { text_block->font = font; /*text_block->widget.need_refresh = true;*/ } }
	
void TextBlock_setForegroundColor(TextBlock *text_block, uint rgb)	
	{ text_block->foreground = rgb; text_block->foreground_changed = true; /*text_block->widget.need_refresh = true;*/ }

void TextBlock_setBackgroundColor(TextBlock *text_block, uint rgb)	
...
void TextBlock_staticGetTextSize(TTF_Font *font, char *text, int *w, int *h) { TTF_SizeUTF8(font, text, w, h); }
