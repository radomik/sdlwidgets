...
static void ButtonImage_vdestroy(void *vthis) {
	...
	/*! Delete self stuff !*/
	uint i = 0; for(; i < 4; i++)	// delete composed labelimages (with their surfaces)
...
ButtonImage* ButtonImage_new(	ButtonImage *this,
	...
	this->enabled				= true;
	uint i=0; for (; i < 4; i++) LabelImage_new(&(this->labelimage[i]), img_path);
	return this;
}
...
void ButtonImage_setBorder(ButtonImage *buttonimage, usint bord_width, uint rgb) 
	{ uint i = 0; for(; i < 4; i++) LabelImage_setBorder(&(buttonimage->labelimage[i]), bord_width, rgb); }

void ButtonImage_setFixedWidth(ButtonImage *buttonimage, bool sel) 
	{ uint i = 0; for(; i < 4; i++)  buttonimage->labelimage[i].fixed_width = sel; }
...
void ButtonImage_setImage(ButtonImage *buttonimage, Image *image) {
	uint i=0; for(;i<4;i++) LabelImage_setImage(&(buttonimage->labelimage[i]), image);
	ButtonImage_refresh(buttonimage);
}

void ButtonImage_scale(ButtonImage *buttonimage, double xscale, double yscale, int smooth) { 
	//Widget_scale(&(buttonimage->widget), xscale, yscale, smooth); 
	uint i=0; for(;i<4;i++) LabelImage_scale(&(buttonimage->labelimage[i]), xscale, yscale, smooth); 
	ButtonImage_refresh(buttonimage);
}
