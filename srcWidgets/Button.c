...
static void Button_vdestroy(void *vthis) {
	...
	/*! Delete self stuff !*/
	uint i = 0; for(; i < 4; i++)	// delete composed labels (with their surfaces)
		delete(&(this->label[i]));
...
void Button_setBgColor(Button *button, uint rgb) 
	{ uint i = 0; for(; i < 4; i++) Label_setBgColor(&(button->label[i]), rgb); }

void Button_setBorder(Button *button, usint bord_width, uint rgb) 
	{ uint i = 0; for(; i < 4; i++) Label_setBorder(&(button->label[i]), bord_width, rgb); }
...
void Button_setFixedWidth(Button *button, bool sel) { 
	uint i = 0; for(; i < 4; i++) button->label[i].fixed_width = sel; 
}

void Button_setText(Button *button, char *text) {
	uint i=0; for(;i<4;i++) Label_setText(&(button->label[i]), text);
