...
char* StackList_getLayoutName(layouttype layout) { return (layout == HORIZONTAL)?"HORIZONTAL":"VERTICAL"; }

static char str_id[80];
char *StackList_toString(StackList *stacklist) {
...
char *StackList_lastWidgetToString(StackList *stacklist) { return (stacklist->count == 0) ? "(null)" : Widget_toString(stacklist->items[stacklist->count-1].widget); }

static void StackList_destroy_common(StackList *this, const char *fname) {
...
static void StackList_new_common(StackList *this, layouttype layout, uint size, const char *fname) {
	if (VERBOSE_CREATE)
...
void StackList_dragItems(StackList *stacklist, signed short dx, signed short dy) {  
...
void StackList_setVisible(StackList *stacklist, bool vis) {
...
void StackList_validate(StackList *stacklist) { 
...
void StackList_validateFrom(StackList *stacklist, uint index) { 
