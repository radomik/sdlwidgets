...
#include "srcGTools/GToolsPoint.h"
#include "srcGTools/GToolsSplotFilter.h"
#include "srcGTools/GToolsFilters.h"
#include "srcGTools/GToolsMorph.h"
...
void button_brightness_change(Widget *sender, Screen *screen) {
	...
	if ((s = GToolsPoint_apply_brightness_contrast(imgsrc, imgdst, *img_mul, *img_off, *allow_of)) != 0)
...
void button_destaturize_std(Widget *sender, Screen *screen) {
	...
	Image *imgdst = (Image*)sender->cparam[1];
	uchar s;
	if ((s = GToolsPoint_desaturize_std(imgsrc->widget.surf, imgdst->widget.surf)) != 0)
 ...
 * sender->cparam[0]  -- label binarization threshold will set to "" at text+7						as Label*
 ...
 * sender->cparam[1]  -- label binarization threshold will set to "" at text+7					as Label*
 * sender->cparam[2]  -- buttonimage binarization threshold up will be disabled						as ButtonImage*
 * sender->cparam[3]  -- buttonimage binarization threshold down will be disabled					as ButtonImage* 
...
void button_create_histograms_click(Widget *sender, Screen *screen) {
	...
	uchar		s;
	if ((s = GToolsPoint_create_histograms(imgsrc->widget.surf, colors)) != 0) {
...
void button_histograms_equalize(Widget *sender, Screen *screen) {
	...
	if ((s = GToolsPoint_histEqualize(imgdst->widget.surf, colors)) != 0) {
...
void button_bin_create_histograms(Widget *sender, Screen *screen) {
	...
	if ((s = GToolsPoint_create_histograms(imgsrc->widget.surf, colors)) != 0) {
		...
		if ((s = GToolsPoint_calculate_binary_threshold(histogram[*num_hist].data, thr_value, *t_algo)) != 0) {
...
void button_threshold_apply(Widget *sender, Screen *screen) {
	...
	uchar	s;
	if (*thr_value <= 255) {
		if ((s = GToolsPoint_apply_bin_threshold(imgsrc->widget.surf, imgdst->widget.surf, *thr_value)) != 0)
...
void button_dest_negative(Widget *sender, Screen *screen) {
	...
	if ((s = GToolsPoint_negative(surf)) != 0)
...
void button_dest_splotfilter(Widget *sender, Screen *screen) {
	...
	uchar s;
	if ((s = GToolsSplotFilter_applyFilter(gtsf, imgsrc->widget.surf, imgdst->widget.surf)) != 0)
...
void button_apply_reconstruction_filter(Widget *sender, Screen *screen) {
	...
	if ((s = GToolsMorph_reconstruction(imgmask->widget.surf, imgmarker->widget.surf, *limit)) != 0) {
...
void button_rep_col_apply(Widget *sender, Screen *screen) {
	...
	uchar 		s;
	if ((s = GToolsPoint_replace_pixels(imgsrc->widget.surf, imgdst->widget.surf, *cond, *thr, new_col)) != 0)
...
void histogram_stretch_on_mouse_release(Widget *sender, Screen *screen) {
	...
	uchar s;
	if ((s = GToolsPoint_histogram_stretching(imgsrc->widget.surf, imgdst->widget.surf, hsg->x, hsg->y, hsg->size)) != 0) {
...
void button_change_hist_stretch_nodes(Widget *sender, Screen *screen) {
	...
	if ((s = HistStretchGraph_refresh2(hsg)) != 0) {
...
void button_generic_filter_apply(Widget *sender, Screen *screen) {
	...
	uchar s;
	if ((s = GToolsFilter_apply(imgsrc->widget.surf, imgdst->widget.surf, type)) != 0)
