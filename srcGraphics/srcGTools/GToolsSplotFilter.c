...
void  GToolsSplotFilter_resetFilter(GToolsSplotFilter *gtsf) { 
	gtsf->predef_filter_ind = USINF; 
}

usint GToolsSplotFilter_getFilterInd(GToolsSplotFilter *gtsf) { 
	return gtsf->predef_filter_ind;
}

const char* GToolsSplotFilter_getFilterName(GToolsSplotFilter *gtsf) { 
	return (gtsf->predef_filter_ind==USINF) ? "" : predef_filter[gtsf->predef_filter_ind].name; 
}

bool GToolsSplotFilter_increaseFilterInd(GToolsSplotFilter *gtsf) { 
...
bool GToolsSplotFilter_decreaseFilterInd(GToolsSplotFilter *gtsf) { 
...
bool GToolsSplotFilter_increaseWeightAt(GToolsSplotFilter *gtsf, uchar w) {
	if (w < 9) { 
...
bool GToolsSplotFilter_decreaseWeightAt(GToolsSplotFilter *gtsf, uchar w) {
	if (w < 9) { 
		if (gtsf->weight[w] == -100.0f) return false; 
		if (gtsf->predef_filter_ind != USINF)
			gtsf->predef_filter_ind = USINF;
...
bool GToolsSplotFilter_increaseDivider(GToolsSplotFilter *gtsf) {
...
bool GToolsSplotFilter_decreaseDivider(GToolsSplotFilter *gtsf) {
	if (gtsf->divider == -100.0f) return false; 
	if (gtsf->predef_filter_ind != USINF)
		gtsf->predef_filter_ind = USINF;
	gtsf->divider -= 0.1f; 
...
float GToolsSplotFilter_getWeightAt(GToolsSplotFilter *gtsf, uchar w) { 
...
uchar GToolsSplotFilter_applyFilter(GToolsSplotFilter *gtsf, SDL_Surface *imgsrc, SDL_Surface *imgdst) {
	...
	if ((imgsrc->w < 3) || (imgsrc->h < 3)) {
		fprintf(stderr, "GToolsSplotFilter_applyFilter() > Input image to small (below 3x3 pixels)\n");
		return 1;
	}
