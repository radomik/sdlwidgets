...
static void Audio_destroy_common(Audio *this, const char *fname) {
...
void Audio_SDL_stop(Audio *audio) { 
	SDL_CloseAudio(); 
...
void Audio_pause(Audio *audio) { if (audio->play) { if (audio->lib == SDL_LIB) Audio_SDL_pause(audio); else Audio_PA_pause(audio); } }
