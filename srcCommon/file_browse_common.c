 ...
 */
bool fileOnMask(char *ext, char (*file_mask)[][MAX_EXT_SIZE]) {
	if (! ext) return false;
	uchar i = 0;
							...
							) 
{
	...
	while ((dir = readdir(d)) != NULL) { 			// for each file in directory
