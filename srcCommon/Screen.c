...
usint Screen_getWidth() { return (vinfo)?(vinfo)->current_w:0; }
usint Screen_getHeight() { return (vinfo)?(vinfo)->current_h:0; }
...
SDL_Surface*			Screen_getBaseSurface()			{ return BASE_SURFACE; }
SDL_PixelFormat*		Screen_getBaseSurfaceFormat()	{ return BASE_SURFACE_FORMAT; }
...
void Screen_setBackgroundColor(Screen *screen, uint rgb) { screen->background.bgcolor = rgb; screen->need_reload = true; }

char *Screen_lastWidgetToString(Screen *screen) { if (screen->c_widget==0)return NULL; return Widget_toString(screen->widget[screen->c_widget-1]); }

/** private method Deinitializes SDL libraries */
static void Screen_MainDestroy() {
...
Screen* Screen_new(Screen *this, perr *e, uint size_widget) {
	...
	if (! BASE_SURFACE) {
		...
		if ((ee = Screen_MainInit()) != E_NO_ERROR) {
...
void Screen_draw(Screen *screen) {
	...
	if ((widget = screen->widget_ontop) != NULL) {
		if (widget->type == CONTAINER_TYPE)
			Container_draw((Container*)widget, screen, false);
		else
			Widget_draw(widget, screen, false);
...
void Screen_buttonExitClicked(Widget *sender, Screen *screen) {
