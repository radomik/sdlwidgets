 ...
 */
	...
	void Screen_setBackgroundColor(Screen *screen, uint rgb);
	
	// Common handler for close window button
	void Screen_buttonExitClicked(Widget *sender, Screen *screen);
	
	// ToStrings()
	char *Screen_lastWidgetToString(Screen *screen);
	...
	SDL_Surface*			Screen_getBaseSurface();
	SDL_PixelFormat*		Screen_getBaseSurfaceFormat();
	
	void Screen_setOptions(options *op);
	usint Screen_getWidth();
	usint Screen_getHeight();
#endif
